...
import { Div, Button, defaultTheme as theme, breakpoints } from '@ui-library';
  ...
  ${({ mobile }) => {
    ...
    if (mobile)
      return css`
        ...
        padding-top: auto;
        padding-bottom: auto;
        background-color: ${theme.colors.globalBackgroundColor};
      `;
  }}
  ${({ mobile, secondary }) => {
...
export const NavSubItem = styled.div<NavSubItemProps>`
  ...
  color: ${theme.palette.greyDark2};
  margin: 0px auto;
  width: auto;

  &:hover {
    background-color: ${theme.colors.cardLightOutline};
  }
...
export const NavSubItemContainer = styled.div<NavSubItemProps>`
  ...
  border-radius: ${theme.radii};
  background-color: ${theme.colors.globalBackgroundColor};
  border-color: ${theme.colors.cardLightOutline};
  text-align: center;
